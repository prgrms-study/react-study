...
하지만  명심해야 될 것은 `useMemo` 는 값을 메모이제이션 하고  `useCallback`은 함수로직을 메모이제이션 하는점이다.

`useCallback`을 값을 반환하는 로직으로로 작성을 했다면 `useMemo`로 단순하게 아래와같이 대체할 수 있다

```jsx
// useCallback
const userLength = useCallback(() => users.length , [users])

// useMemo
const userLength = useMemo(() => users.length , [users])
```



하지만 `useCallback`의 로직이 값만을 반환하는게 아닌 어떠한 다른 복잡한 처리를 하는 로직이 있다면 대체하기에는 적절하지 않다.
...
위 함수의 로직은 값을 반환받는게 아닌 상태의 변경이 초점이다. 이것을 굳이 useMemo로 바꿀필요가 있어보이는가? 아닐것이다.
